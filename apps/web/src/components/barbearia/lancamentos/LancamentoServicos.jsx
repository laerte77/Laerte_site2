...
import { Plus, Loader2, Trash, Scissors, Search, Download, Edit } from 'lucide-react';
...
const LancamentoServicos = () => {
    ...
    const fetchData = async () => {
        ...
        setLoadingData(true);
        try {
            const [resClientes, resServicos, resBarbeiros, resLancamentos] = await Promise.all([
                ...
                supabase.from('barbearia_lancamentos_servicos').select('*, barbearia_clientes(nome), barbearia_servicos(nome), barbearia_barbeiros(nome)').eq('user_id', user.id).order('data', { ascending: false })
            ]);
    ...
    const filteredLancamentos = useMemo(() => {
        return lancamentos.filter(item => {
            ...
            const sMatch = !searchTerm || 
                ...
                item.barbearia_servicos?.nome?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const handleSave = async () => {
        if (!formData.data || !formData.servico_id || !formData.cliente_id || !formData.valor || !formData.barbeiro_id) { 
        ...
        try {
            if (editingId) {
                await supabase.from('barbearia_lancamentos_servicos').update(payload).eq('id', editingId);
                toast({ title: 'Sucesso', description: 'Serviço atualizado!' });
            } else {
                await supabase.from('barbearia_lancamentos_servicos').insert(payload);
                toast({ title: 'Sucesso', description: 'Serviço lançado!' }); 
            }
    ...
    const handleDelete = async (id) => {
        await supabase.from('barbearia_lancamentos_servicos').delete().eq('id', id);
        fetchData();
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredLancamentos.map(item => ({
            ...
            'SERVIÇO': item.barbearia_servicos?.nome || '-',
        ...
        XLSX.writeFile(workbook, `Servicos_Barbearia_${meses[selectedMonth]}_${selectedYear}.xlsx`);
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 py-8 dark-barbearia">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-[#D4AF37]">Lançamento de Serviços</h1>
                  <p className="text-muted-foreground mt-1 text-lg">Registre serviços avulsos realizados.</p>
                </div>
                ...
                <DialogContent className="dark-barbearia bg-card border-[#D4AF37]/50 text-foreground sm:max-w-[500px]">
                    <DialogHeader>
                        <DialogTitle className="text-2xl text-[#D4AF37] flex items-center gap-2">
                            <Scissors className="w-6 h-6" /> {editingId ? 'Editar' : 'Novo'} Serviço
                        </DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        ...
                        <div>
                            <Label>Serviço *</Label>
                            <Select value={formData.servico_id} onValueChange={v=>setFormData({...formData, servico_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{servicos.map(t => <SelectItem key={t.id} value={t.id}>{t.nome}</SelectItem>)}</ScrollArea></SelectContent></Select>
                        </div>
                    </div>
              ...
              <Card className="col-span-1 md:col-span-3 shadow-md border-border rounded-2xl bg-card">
                <CardContent className="p-5 flex flex-col md:flex-row gap-4 items-center">
                  <div className="flex items-center gap-2 flex-1 w-full relative">
                    ...
                    <Input placeholder="Buscar por cliente, serviço ou barbeiro..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 pl-10 bg-input h-12 text-base text-foreground"/>
                  </div>
                        ...
                        <Table>
                            <TableHeader className="bg-secondary/50 sticky top-0 z-10 backdrop-blur-sm border-b border-border">
                                <TableRow className="hover:bg-transparent">
                                  ...
                                  <TableHead className="py-4 font-semibold">Serviço</TableHead>
                                ...
                                {loadingData ? (<tr><td colSpan="6" className="text-center py-16 text-muted-foreground"><Loader2 className="w-8 h-8 animate-spin mx-auto text-[#D4AF37]" /></td></tr>) : filteredLancamentos.length === 0 ? (<tr><td colSpan="6" className="text-center py-16 text-muted-foreground">Nenhum registro encontrado.</td></tr>) : (
                                    filteredLancamentos.map((item) => (
                                        <TableRow key={item.id} className="border-b border-border hover:bg-secondary/50 transition-colors duration-200">
                                            ...
                                            <TableCell className="py-4 text-muted-foreground">{item.barbearia_servicos?.nome}</TableCell>
