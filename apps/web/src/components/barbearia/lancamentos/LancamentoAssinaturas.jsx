...
import { Plus, Award, Loader2, Trash, Search, Download, Edit } from 'lucide-react';
...
const LancamentoAssinaturas = () => {
    ...
    const initialFormData = { data_assinatura: format(new Date(), 'yyyy-MM-dd'), plano_id: '', cliente_id: '', valor: '' };
    const [formData, setFormData] = useState(initialFormData);

    const availableYears = useMemo(() => {
        const years = lancamentos.map(d => {
            if (!d.data_assinatura) return new Date().getFullYear();
            return Number(d.data_assinatura.split('T')[0].split('-')[0]);
        });
    ...
    const fetchData = async () => {
        ...
        setLoadingData(true);
        try {
            const [resClientes, resPlanos, resLancamentos] = await Promise.all([
                supabase.from('barbearia_clientes').select('id, nome').eq('user_id', user.id).order('nome'),
                supabase.from('barbearia_tipos_planos').select('id, nome, valor').eq('user_id', user.id).order('nome'),
                supabase.from('barbearia_lancamentos_assinaturas').select('*, barbearia_clientes(nome), barbearia_tipos_planos(nome)').eq('user_id', user.id).order('data_assinatura', { ascending: false })
            ]);
    ...
    const filteredLancamentos = useMemo(() => {
        return lancamentos.filter(item => {
            if (!item.data_assinatura) return false;
            const [y, m] = item.data_assinatura.split('T')[0].split('-');
            ...
            const sMatch = !searchTerm || 
                ...
                item.barbearia_tipos_planos?.nome?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const handleSave = async () => {
        if (!formData.data_assinatura || !formData.plano_id || !formData.cliente_id || !formData.valor) { 
            toast({ title: 'Atenção', description: 'Preencha todos os campos.', variant: 'destructive' }); return; 
        }
        setSaving(true);
        const payload = { ...formData, valor: parseFloat(formData.valor)||0, user_id: user.id, status: 'ativa' };
        try {
            if (editingId) {
                await supabase.from('barbearia_lancamentos_assinaturas').update(payload).eq('id', editingId);
                toast({ title: 'Sucesso', description: 'Assinatura atualizada!' });
            } else {
                await supabase.from('barbearia_lancamentos_assinaturas').insert(payload);
                toast({ title: 'Sucesso', description: 'Assinatura registrada!' });
            }
            setFormData(prev => ({...initialFormData, data_assinatura: prev.data_assinatura}));
            setEditingId(null);
            fetchData();
    ...
    const handleDelete = async (id) => {
        await supabase.from('barbearia_lancamentos_assinaturas').delete().eq('id', id);
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredLancamentos.map(item => ({
            'DATA': formatDateDisplay(item.data_assinatura),
            'CLIENTE': item.barbearia_clientes?.nome || '-',
            'PLANO': item.barbearia_tipos_planos?.nome || '-',
            'VALOR': parseFloat(item.valor)
        }));
        const worksheet = XLSX.utils.json_to_sheet(dataToExport);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Assinaturas");
        XLSX.writeFile(workbook, `Assinaturas_Barbearia_${meses[selectedMonth]}_${selectedYear}.xlsx`);
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 py-8 dark-barbearia">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-[#D4AF37]">Assinaturas e Planos</h1>
                  <p className="text-muted-foreground mt-1 text-lg">Gerencie a venda de planos para clientes.</p>
                </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={(o) => { if(!o) setIsDialogOpen(false); }}>
                <DialogContent className="dark-barbearia bg-card border-[#D4AF37]/50 text-foreground sm:max-w-[450px]">
                    <DialogHeader><DialogTitle className="text-2xl text-[#D4AF37] flex items-center gap-2"><Award className="w-6 h-6"/> {editingId ? 'Editar' : 'Nova'} Assinatura</DialogTitle></DialogHeader>
                    <div className="space-y-4 py-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div><Label>Data *</Label><Input type="date" value={formData.data_assinatura} onChange={e=>setFormData({...formData, data_assinatura:e.target.value})} className="bg-input border-border text-foreground" /></div>
                        ...
                        <div><Label>Plano *</Label><Select value={formData.plano_id} onValueChange={v=>setFormData({...formData, plano_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{planos.map(p => <SelectItem key={p.id} value={p.id}>{p.nome}</SelectItem>)}</ScrollArea></SelectContent></Select></div>
              ...
              <Card className="col-span-1 md:col-span-3 shadow-md border-border rounded-2xl bg-card">
                <CardContent className="p-5 flex flex-col md:flex-row gap-4 items-center">
                  <div className="flex items-center gap-2 flex-1 w-full relative">
                    ...
                    <Input placeholder="Buscar por cliente ou plano..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 pl-10 bg-input h-12 text-base text-foreground"/>
                  </div>
                ...
                <CardContent className="p-0">
                    <ScrollArea className="h-[500px]">
                        <Table>
                            ...
                            <TableBody>
                                {loadingData ? (<tr><td colSpan="5" className="text-center py-16 text-muted-foreground"><Loader2 className="w-8 h-8 animate-spin mx-auto text-[#D4AF37]" /></td></tr>) : filteredLancamentos.length === 0 ? (<tr><td colSpan="5" className="text-center py-16 text-muted-foreground">Nenhum registro encontrado.</td></tr>) : (
                                    filteredLancamentos.map((item) => (
                                        <TableRow key={item.id} className="border-b border-border hover:bg-secondary/50 transition-colors duration-200">
                                            <TableCell className="font-medium pl-6 py-4">{formatDateDisplay(item.data_assinatura)}</TableCell>
                                            <TableCell className="py-4 font-medium text-foreground">{item.barbearia_clientes?.nome}</TableCell>
                                            <TableCell className="py-4 text-muted-foreground">{item.barbearia_tipos_planos?.nome}</TableCell>
...
export default LancamentoAssinaturas;