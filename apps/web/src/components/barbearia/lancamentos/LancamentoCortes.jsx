...
import { Plus, Scissors, Loader2, Trash, Search, Download, Edit } from 'lucide-react';
...
const LancamentoCortes = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [resClientes, resCortes, resServicos, resBarbeiros, resLancamentos] = await Promise.all([
                ...
                supabase.from('barbearia_lancamentos_cortes').select('*, barbearia_clientes(nome), barbearia_tipos_corte(nome), barbearia_barbeiros(nome)').eq('user_id', user.id).order('data', { ascending: false })
            ]);
            setClientes(resClientes.data || []);
            setTiposCorte(resCortes.data || []);
    ...
    const filteredLancamentos = useMemo(() => {
        return lancamentos.filter(item => {
            ...
            const sMatch = !searchTerm || 
                ...
                item.barbearia_barbeiros?.nome?.toLowerCase().includes(searchTerm.toLowerCase()) || 
                item.barbearia_tipos_corte?.nome?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const openDialog = (item = null) => {
        if (item) {
            ...
            setFormData({
                ...
                tem_servico: item.tem_servico || false,
    ...
    const handleSave = async () => {
        if (!formData.data || !formData.tipo_corte_id || !formData.cliente_id || !formData.valor || !formData.barbeiro_id) { 
            toast({ title: 'Atenção', description: 'Preencha todos os campos obrigatórios.', variant: 'destructive' }); 
            return; 
        }
        if (formData.tem_servico && !formData.servico_id) {
            toast({ title: 'Atenção', description: 'Selecione o serviço adicional.', variant: 'destructive' }); 
            return;
        }
        ...
        const payload = { 
            ...
            servico_id: formData.tem_servico ? formData.servico_id : null,
            user_id: user.id 
        };

        try {
            if (editingId) {
                await supabase.from('barbearia_lancamentos_cortes').update(payload).eq('id', editingId);
                toast({ title: 'Sucesso', description: 'Corte atualizado com sucesso!' });
            } else {
                await supabase.from('barbearia_lancamentos_cortes').insert(payload);
                toast({ title: 'Sucesso', description: 'Corte lançado com sucesso!' }); 
            }
    ...
    const handleDelete = async (id) => {
        await supabase.from('barbearia_lancamentos_cortes').delete().eq('id', id);
        fetchData();
    ...
    const handleExport = () => {
        ...
        const dataToExport = filteredLancamentos.map(item => ({
            ...
            'CORTE': item.barbearia_tipos_corte?.nome || '-',
        ...
        XLSX.writeFile(workbook, `Cortes_Barbearia_${meses[selectedMonth]}_${selectedYear}.xlsx`);
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 py-8 dark-barbearia">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-[#D4AF37]">Lançamento de Cortes</h1>
                  <p className="text-muted-foreground mt-1 text-lg">Registre os cortes e serviços realizados.</p>
                </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={(o) => { if(!o) setIsDialogOpen(false); }}>
                <DialogContent className="dark-barbearia bg-card border-[#D4AF37]/50 text-foreground sm:max-w-[500px]">
                    <DialogHeader>
                        <DialogTitle className="text-2xl text-[#D4AF37] flex items-center gap-2">
                            <Scissors className="w-6 h-6" /> {editingId ? 'Editar' : 'Novo'} Corte
                        </DialogTitle>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div><Label>Data *</Label><Input type="date" value={formData.data} onChange={e=>setFormData({...formData, data:e.target.value})} className="bg-input border-border" /></div>
                            <div><Label>Valor (R$) *</Label><Input type="number" step="0.01" value={formData.valor} onChange={e=>setFormData({...formData, valor:e.target.value})} className="bg-input border-border" placeholder="0.00" /></div>
                        </div>
                        <div>
                            <Label>Cliente *</Label>
                            <Select value={formData.cliente_id} onValueChange={v=>setFormData({...formData, cliente_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{clientes.map(c => <SelectItem key={c.id} value={c.id}>{c.nome}</SelectItem>)}</ScrollArea></SelectContent></Select>
                        </div>
                        <div>
                            <Label>Barbeiro *</Label>
                            <Select value={formData.barbeiro_id} onValueChange={v=>setFormData({...formData, barbeiro_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{barbeiros.map(b => <SelectItem key={b.id} value={b.id}>{b.nome}</SelectItem>)}</ScrollArea></SelectContent></Select>
                        </div>
                        <div>
                            <Label>Tipo de Corte *</Label>
                            <Select value={formData.tipo_corte_id} onValueChange={v=>setFormData({...formData, tipo_corte_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{tiposCorte.map(t => <SelectItem key={t.id} value={t.id}>{t.nome}</SelectItem>)}</ScrollArea></SelectContent></Select>
                        </div>
                        <div className="flex items-center justify-between bg-muted/50 p-3 rounded-lg border border-border">
                            <Label className="cursor-pointer font-medium" htmlFor="tem-servico">Adicionar Serviço Extra?</Label>
                            <Switch id="tem-servico" checked={formData.tem_servico} onCheckedChange={c => setFormData({...formData, tem_servico:c})} />
                        </div>
                        {formData.tem_servico && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="pt-2">
                                ...
                                <Select value={formData.servico_id} onValueChange={v=>setFormData({...formData, servico_id:v})}><SelectTrigger className="bg-input"><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent className="dark-barbearia bg-card"><ScrollArea className="h-48">{servicos.map(s => <SelectItem key={s.id} value={s.id}>{s.nome}</SelectItem>)}</ScrollArea></SelectContent></Select>
                            </motion.div>
                        )}
                    </div>
                    <DialogFooter className="mt-4 gap-2">
                        <Button variant="outline" onClick={()=>setIsDialogOpen(false)} disabled={saving}>Cancelar</Button>
                        <Button onClick={handleSave} disabled={saving} className="bg-[#D4AF37] hover:bg-[#B5952F] text-black font-bold flex-1">{saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null} Salvar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
              ...
              <Card className="col-span-1 md:col-span-3 shadow-md border-border rounded-2xl bg-card">
                <CardContent className="p-5 flex flex-col md:flex-row gap-4 items-center">
                  <div className="flex items-center gap-2 flex-1 w-full relative">
                    ...
                    <Input placeholder="Buscar por cliente, corte ou barbeiro..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 pl-10 bg-input h-12 text-base text-foreground"/>
                  </div>
                        ...
                        <Table>
                            <TableHeader className="bg-secondary/50 sticky top-0 z-10 backdrop-blur-sm border-b border-border">
                                <TableRow className="hover:bg-transparent">
                                  ...
                                  <TableHead className="py-4 font-semibold">Corte</TableHead>
                                ...
                                {loadingData ? (<tr><td colSpan="6" className="text-center py-16 text-muted-foreground"><Loader2 className="w-8 h-8 animate-spin mx-auto text-[#D4AF37]" /></td></tr>) : filteredLancamentos.length === 0 ? (<tr><td colSpan="6" className="text-center py-16 text-muted-foreground">Nenhum registro encontrado.</td></tr>) : (
                                    filteredLancamentos.map((item) => (
                                        <TableRow key={item.id} className="border-b border-border hover:bg-secondary/50 transition-colors duration-200">
                                            ...
                                            <TableCell className="py-4 text-muted-foreground">{item.barbearia_tipos_corte?.nome}</TableCell>
...
export default LancamentoCortes;