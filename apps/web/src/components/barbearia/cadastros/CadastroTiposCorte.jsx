...
const CadastroTiposCorte = () => {
    ...
    const fetchData = async () => {
        ...
        setLoading(true);
        try {
            const { data: res } = await supabase.from('barbearia_tipos_corte').select('*').eq('user_id', user.id).order('nome');
            setData(res || []);
    ...
    const handleSave = async () => {
        ...
        try {
            if (currentItem) await supabase.from('barbearia_tipos_corte').update(payload).eq('id', currentItem.id);
            else await supabase.from('barbearia_tipos_corte').insert(payload);
            toast({ title: 'Sucesso', description: 'Corte salvo.' }); 
            fetchData(); 
    ...
    const handleDelete = async (id) => {
        await supabase.from('barbearia_tipos_corte').delete().eq('id', id);
        fetchData();
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-[#D4AF37] uppercase">Tipos de Corte</h2>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="bg-gray-900 border-[#D4AF37]/50 text-white">
                    <DialogHeader><DialogTitle className="text-[#D4AF37]">{currentItem ? 'Editar' : 'Novo'} Tipo de Corte</DialogTitle></DialogHeader>
                    <div className="space-y-4 py-4">
                        <div><Label className="text-[#A9A9A9]">Nome do Corte *</Label><Input value={formData.nome} onChange={e=>setFormData({...formData, nome:e.target.value})} className="bg-black border-[#D4AF37]/30 text-white" /></div>
...
export default CadastroTiposCorte;