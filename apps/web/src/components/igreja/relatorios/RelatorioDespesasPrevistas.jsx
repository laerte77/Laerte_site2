...
import { FileSearch, CheckCircle, FileClock } from 'lucide-react';
...
const RelatorioDespesasPrevistas = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const [previstasRes, tiposRes] = await Promise.all([
            ...
            if (previstasRes.error) throw previstasRes.error;
            setDespesasPrevistas(previstasRes.data || []);
    ...
    useEffect(() => {
        fetchData();
        if (!user) return;
        const channel = supabase.channel('relatorio_igreja_despesas_previstas_changes_v2')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'igreja_despesas_previstas' }, () => {
        ...
        despesasPrevistas.forEach(dp => {
            if (dp.forma_pagamento === 'CARTÃO DE CRÉDITO' && dp.parcelas > 1) {
                ...
                for (let i = 0; i < dp.parcelas; i++) {
                    ...
                    allInstallments.push({ ...dp, id: installmentId, valor: valorParcela, vencimento: vencimento.toISOString().split('T')[0], descricao: `${dp.despesa} (${i + 1}/${dp.parcelas})`, original_id: dp.id, parcela_num: i + 1, status: parcelaStatus });
                }
            } else {
                allInstallments.push({ ...dp, original_id: dp.id, descricao: dp.despesa, status: dp.status === 'PAGO' ? 'PAGO' : 'PENDENTE' });
            }
        });
    ...
    const filteredDespesas = useMemo(() => {
        return processedDespesas.filter(d => {
            ...
            if (filters.mes !== 'todos' && dataVencimento.getMonth() !== parseInt(filters.mes)) return false;
            if (filters.tipo !== 'todos' && d.despesa !== filters.tipo) return false;
            return true;
    ...
    const toggleStatus = async (item) => {
        ...
        const { error } = await supabase.from('igreja_despesas_previstas').update(newStatusValue).eq('id', item.original_id);
        ...
        >
             <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Vencimento: {new Date(item.vencimento).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</p>
                    {item.status !== 'PAGO' && <p className="text-xs font-medium text-orange-400 mt-0.5">{calculateDaysRemaining(item.vencimento)}</p>}
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-20 md:pb-0">
            ...
            <div className="bg-card/80 p-4 md:p-6 rounded-xl border border-border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                    ...
                    <div>
                        <Label><span>Filtrar por Tipo de Despesa</span></Label>
                        <Select value={filters.tipo} onValueChange={v => setFilters({ ...filters, tipo: v })}>
                            <SelectTrigger className="bg-input text-foreground h-11"><SelectValue /></SelectTrigger>
                            <SelectContent className="dark-igreja">
                                <ScrollArea className="h-48">
                                    <SelectItem value="todos"><span>Todos os Tipos</span></SelectItem>
                            ...
                            {loading ? (
                                ...
                                filteredDespesas.map((item) => (
                                    <tr key={item.id} className="border-b border-border last:border-b-0 hover:bg-accent/50 text-foreground">
                                        ...
                                        <td className="p-4"><span>{new Date(item.vencimento).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</span></td>
                                        <td className="p-4"><span>{item.status !== 'PAGO' ? calculateDaysRemaining(item.vencimento) : '-'}</span></td>
